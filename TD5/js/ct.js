...
async function getData(pagination) {
    ...
    if(ville === undefined){
        url = "https://data.economie.gouv.fr/api/records/1.0/search/?dataset=prix-des-controles-techniques-copie&q=&rows=" + nbRows + "&start=" + (start-1) + "&facet=cct_denomination&facet=prix_visite&facet=prix_contre_visite_min&facet=prix_contre_visite_max&refine.cct_code_dept=" + dpt;
    }
    else{
        url = "https://data.economie.gouv.fr/api/records/1.0/search/?dataset=prix-des-controles-techniques-copie&q=&rows=" + nbRows + "&start=" + (start-1) + "&facet=cct_denomination&facet=prix_visite&facet=prix_contre_visite_min&facet=prix_contre_visite_max&refine.cct_code_commune=" + ville;
    }
    ...
    if(json.nhits !== 0){
        ...
        json.records.forEach(centre =>{
            ...
            tr.append(nom, adresse, prix_visite, prix_contre_visite_max, prix_contre_visite_min);
